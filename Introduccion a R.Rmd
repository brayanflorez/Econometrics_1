---
title: "Clase 01: Introducción a R"
...
| Rol                         | Nombre               | Correo                         |
|-----------------------------|----------------------|--------------------------------|
| Profesor magistral          | Edicson Luna         | edicson@sas.upenn.edu          |
| Asistente / Clase compl.    | Brayan David Flórez  | b.florezl@uniandes.edu.co      |
...
| Semana | Temas (clase magistral)                                 | Evaluación              |
|--------|---------------------------------------------------------|-------------------------|
| 1      | Repaso de estadística                                   | —                       |
| 2      | Repaso de estadística, estimador OLS                    | —                       |
| 3      | Insesgamiento, inferencia estadística                   | Quiz 1 (mié. 24 jun.)   |
| 4      | Distintos tipos de variables, teoría asintótica         | Quiz 2 (mié. 1 jul.)    |
| 5      | Principios de inferencia causal                         | Quiz 3 (mié. 15 jul.)   |
| 6      | Variable dicotómica dependiente, presentaciones         | Examen Final (jue. 23 jul.) |

> **Presentaciones:** viernes 24 de julio (clase complementaria)
...
|---------------------------|---------------|---------------|---------------|---------------|
| Costo                     | **Gratuito**  | De pago (~$500/año) | **Gratuito** | De pago      |
...
Este mismo documento fue generado con **R Markdown**: combina texto y código R en un archivo `.Rmd`. Al compilarlo (*knit*), produce un reporte con todos los resultados actualizados automáticamente.

**Ventajas de R Markdown:**

- Integra análisis y escritura en un solo lugar.
- Cualquier persona puede re-ejecutar el código y obtener los mismos resultados.
- Produce reportes en HTML, PDF o Word.
- Es el estándar en investigación reproducible y en muchos equipos de análisis en empresas y entidades públicas.

---

# ¿Por qué usar R?

## R vs. otros programas

Además de lo que vimos en los gráficos, R tiene ventajas técnicas concretas:

- **Más de 20.000 paquetes** en CRAN para tareas especializadas (econometría, machine learning, visualización, etc.).
- **ggplot2**: el estándar de visualización de datos más usado en el mundo académico y empresarial.
- **tidyverse**: ecosistema de paquetes para manipulación de datos de forma clara y eficiente.
- **R Markdown / Quarto**: reportes reproducibles y dinámicos.
- **Shiny**: aplicaciones web interactivas con datos, sin necesidad de saber HTML/CSS.
...
```
┌─────────────────────┬─────────────────────┐
│  Editor de scripts  │  Environment /       │
│  (Scripts .R / .Rmd)│  History / Files     │
├─────────────────────┼─────────────────────┤
│  Consola (Console)  │  Plots / Packages /  │
│                     │  Help / Viewer       │
└─────────────────────┴─────────────────────┘
```
...
# Alternativa recomendada: usar Proyectos de RStudio
# File → New Project → New Directory
```

> **Recomendación:** usa siempre un **Proyecto de RStudio** (`.Rproj`) por curso/trabajo. Esto resuelve automáticamente los problemas de rutas.
...
Los `#` son **comentarios**: R los ignora al ejecutar el código. Úsalos siempre para documentar tu trabajo.
...
# También puedes usar =, aunque <- es la convención
nombre <- "Econometría 1"
nombre
```

El panel **Environment** (arriba a la derecha en RStudio) muestra todos los objetos creados en la sesión.
...
5. **Google y Stack Overflow son tus amigos.** La mayoría de errores ya los sufrió alguien más.
6. **No copies y pegues sin entender.** Reproducibilidad ≠ copy-paste ciego.
